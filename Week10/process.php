...
/*Check to see if the form was submitted from the installed domain. If so, process the data. If not kill the script. You can disable this, but it's strongly recommended that you keep this check in place. */
...
if ( $domain == $r_domain ) {
	...
	/*Open the connection to our dataase use the info from the config file*/
	...
	/*These are the extra dat fields we'll collect on form submission*/
	...
	/*Check to see if the fields specified in teh form exist and if they don't, create them.*/
	...
	/*Closer our database connection.*/
	mysql_close();
  ...
  die ('Your are not allowed to submit data to this form');
}
